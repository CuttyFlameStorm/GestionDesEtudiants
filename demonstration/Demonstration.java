...
import fr.eni.ecole.poo.groupeeleves.entite.Bus;
...
import fr.eni.ecole.poo.groupeeleves.entite.Manege;
...
public class Demonstration {
	...
	private static Manege<Eleve> chenille;
	private static Manege<Parent> tetenbas;

	/**
	 * Création des éléves, parents (référents), instituteurs et classes. Mise en
	...
	/**
	 * Récupération d'un éléve avec un index trop grand. Ceci provoque une exception
	 * qui doit étre gérée
	 */
	...
	/**
	 * Création d'un bus scrolaire de 12 places
	 * @throws BusException 
	 */
	private static void creerUnBus(){
			bus1 = new Bus(12);
		}
	...
	/**
	 * remplissage du bus. Si le bus est plein, les personnes sont refusée par un mécanisme
	 * d'exception
	 */
	private static void remplirBus() throws BusException{
		System.out.println("Remplissage du bus de 12 personnes");
		try {
			bus1.addPersonne(classeRouge.getInstituteur());
		} catch (BusException ex) {
			System.out.println("Problème capacité du Bus et de siège");
		}
		bus1.addPersonne(classeRouge.getInstituteur());

		try {
			bus1.addPersonne(classeRouge.getEleve(0).getReferent());
		} catch (BusException | ClasseException ex) {
			System.err.println(ex.getMessage());
		}

		for (Eleve e : classeRouge.getLstEleves()) {
			try {
				bus1.addPersonne(e);
			} catch (BusException ex) {
				System.err.println(ex.getMessage());
		...
		for (Eleve e : classeVerte.getLstEleves()) {
			try {
				bus1.addPersonne(e);
			} catch (BusException ex) {
				System.err.println(ex.getMessage());
	...
	/**
	 * Lister les personnes responsables dans le bus
	 * @throws BusException 
	 * @throws ClasseException 
	 */
	private static void listeDesPersonnesResponsables() throws BusException{
		System.out.println("Liste des Personnes responsables dans le bus");
			Bus affiche = bus1.afficherResponsable();
			try {	
				bus1.afficherResponsable();
	...
	/**
	 * Création d'un manège pour enfants et un manège pour parents
	 */
	private static void creerManeges() {
		chenille = new Manege<Eleve>("La Chenille");		
		tetenbas = new Manege<Parent>("La Tête en Bas");
		
		// Répartission des occupants du bus dans les manèges
		for (Personne occupant : bus1.getLstPersonnes()) {
			if(occupant instanceof Eleve){
				chenille.addParticipant((Eleve) occupant);
			}
			if(occupant instanceof Parent){
				tetenbas.addParticipant((Parent)occupant);
			}
		}
		
		System.out.println(chenille);
		System.out.println(tetenbas);
	}
